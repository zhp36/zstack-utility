...
class DumpMysqlCmd(Command):
    ...
    def install_argparse_arguments(self, parser):
        ...
        parser.add_argument('--host-info','--h','--host',
                           ...
                           required=False)
        parser.add_argument('--remote-dir','--dir',
                            help="The remote dir to store database and ui backup file default is '/var/lib/zstack/zstack-remote-backup/'",
                            default="/var/lib/zstack/zstack-remote-backup/",
                            required=False)

    def sync_local_backup_db_to_remote_host(self, args, private_key, remote_host_ip):
        ...
        sync_command = "rsync -lr --delete -e 'ssh -i %s'  %s %s %s:%s" % (private_key, self.mysql_backup_dir,
                                                                           self.ui_backup_dir, remote_host_ip, args.remote_dir)
    ...
    def run(self, args):
        ...
        #remote backup
        if args.host_info is not None:
            self.sync_local_backup_db_to_remote_host(args, private_key, remote_host_ip)
            info("Sync ZStack backup to remote host %s:%s successfully! " % (remote_host_ip, args.remote_dir))
